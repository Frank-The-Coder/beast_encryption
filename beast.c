...
static time_t beast_now_time = 0;
 ...
 */
const zend_function_entry beast_functions[] = {
    PHP_FE(beast_encode_file, arginfo_beast_encode_file)
        PHP_FE(beast_avail_cache, arginfo_beast_avail_cache)
            PHP_FE(beast_support_filesize, arginfo_beast_support_filesize)
                PHP_FE(beast_file_expire, arginfo_beast_file_expire) PHP_FE(
                    beast_clean_cache, arginfo_beast_clean_cache) PHP_FE_END};
/* }}} */
...
zend_module_entry beast_module_entry = {
    STANDARD_MODULE_HEADER, "beast",
    beast_functions,        PHP_MINIT(beast),
    PHP_MSHUTDOWN(beast),   PHP_RINIT(beast),
    PHP_RSHUTDOWN(beast),   PHP_MINFO(beast),
    BEAST_VERSION,          STANDARD_MODULE_PROPERTIES};
/* }}} */
...
static struct file_handler *file_handlers[] = {&tmpfile_handler,
#ifndef PHP_WIN32
                                               &pipe_handler,
#endif
                                               NULL};
...
int filter_code_comments(zend_string *filename, zval *retval) {
...
int encrypt_file(const char *inputfile, const char *outputfile, int expire,
                 int encrypt_type) {
  ...
  /* 将 inputfile 转换为 zend_string* 类型 */
  zend_string *inputfile_zstr =
      zend_string_init(inputfile, strlen(inputfile), 0);

  /* 获取 PHP 脚本文件中的代码 */
  if (filter_code_comments(inputfile_zstr, &codes) == -1) {
    php_error_docref(NULL, E_ERROR, "Unable get codes from php file `%s'",
                     ZSTR_VAL(inputfile_zstr));
    zend_string_release(inputfile_zstr);
    return -1;
  }
...
#if ZEND_MODULE_API_NO >= 20151012
  inlen = (int)Z_STRLEN(codes);  // Explicitly cast size_t to int
  inbuf = Z_STRVAL(codes);
#else
  inlen = (int)codes.value.str.len;  // Explicitly cast size_t to int
  inbuf = codes.value.str.val;
#endif

  /* PHP 文件大小不能超过 beast_max_filesize */
...
#if ZEND_MODULE_API_NO >= 20151012
  output_stream =
      php_stream_open_wrapper((char *)outputfile, "w+", REPORT_ERRORS, NULL);
#else
  output_stream = php_stream_open_wrapper(
      (char *)outputfile, "w+", ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL);
#endif

  if (!output_stream) {
    php_error_docref(NULL, E_ERROR, "Unable to open file `%s'", outputfile);
    goto failed;
  }

  /* 如果计算机是小端字节序，则将文件大小更改为大端字节序 */
  if (little_endian()) {
  ...
  php_stream_write(output_stream, encrypt_file_header_sign,
  ...
  if (encrypt_ops->encrypt(inbuf, inlen, &outbuf, &outlen) == -1) {
    php_error_docref(NULL, E_ERROR, "Unable to encrypt file `%s'", outputfile);
    goto failed;
  }
...
failed:
  if (output_stream) php_stream_close(output_stream);
  if (need_free_code) zval_dtor(&codes);
  return -1;
}
...
int decrypt_file(const char *filename, int stream, char **retbuf, int *retlen,
                 int *free_buffer, struct beast_ops **ret_encrypt) {
  cache_key_t findkey;
  cache_item_t *cache;
  ...
  findkey.inode =
      fileinfo.nFileIndexHigh * (MAXDWORD + 1) + fileinfo.nFileIndexLow;

  ull.LowPart = fileinfo.ftLastWriteTime.dwLowDateTime;
  ull.HighPart = fileinfo.ftLastWriteTime.dwHighDateTime;

  findkey.mtime = (int)(ull.QuadPart / 10000000ULL - 11644473600ULL);
  ...
  *free_buffer = 0; /* Set free buffer flag to false */
...
#ifdef PHP_WIN32
  if ((n = _read(stream, header, headerlen)) != headerlen) {
#else
  ...
  if (memcmp(header, encrypt_file_header_sign, encrypt_file_header_length)) {
    if (log_normal_file) {
      beast_write_log(beast_log_error, "File `%s' isn't a encrypted file",
                      filename);
    }
  ...
  if (beast_max_filesize > 0 && reallen > beast_max_filesize) {
    beast_write_log(beast_log_error, "File size `%d' out of max size `%d'",
  ...
  *ret_encrypt = encrypt_ops = beast_get_encrypt_algo(entype);
  ...
  if (!(buffer = malloc(bodylen))) {
    beast_write_log(beast_log_error,
                    "Failed to alloc memory to file `%s' size `%d'", filename,
                    bodylen);
    retval = -1;
    goto failed;
  }
  ...
  /* 2) Read file stream */
#ifdef PHP_WIN32
  if (_read(stream, buffer, bodylen) != bodylen) {
#else
  if (read(stream, buffer, bodylen) != bodylen) {
#endif
    beast_write_log(beast_log_error, "Failed to readed stream from file `%s'",
                    filename);
    retval = -1;
    goto failed;
  }
...
#ifdef PHP_WIN32
        if (_access(temp, F_OK) == -1) {
#else
        if (access(temp, F_OK) == -1) {
#endif
          if (mkdir(temp, 0777) != 0) {
            beast_write_log(beast_log_error,
                            "Failed to make new directory `%s'", temp);
            return -1;
          }
        }
      }
 ...
 * CGI compile file
 */
zend_op_array *cgi_compile_file(zend_file_handle *h, int type) {
  char *buf;
  int fd;
  FILE *fp = NULL;
...
#else
  fp = fopen(ZSTR_VAL(h->filename),
             "rb");  // Convert zend_string to C-style string
#endif
  ...
  retval =
      decrypt_file(ZSTR_VAL(h->filename), fd, &buf, &size, &free_buffer, &ops);
  ...
  if (retval == -1) {
    ...
    goto final;
#else
    php_error_docref(NULL, E_ERROR, "Not allow execute normal PHP script");
    return NULL;
#endif
  }
  ...
  if (h->type == ZEND_HANDLE_FP) {
    fclose(h->handle.fp);
  } else if (h->type == ZEND_HANDLE_STREAM) {
    if (h->handle.stream.closer) {
  ...
  switch (default_file_handler->type) {
    ...
    case BEAST_FILE_HANDLER_FD:
      h->type = ZEND_HANDLE_STREAM;
      h->handle.stream.handle = (void *)(intptr_t)default_file_handler->get_fd(
          default_file_handler);  // Cast to void*
      h->handle.stream.reader =
          NULL;  // You should provide a reader function if necessary
      h->handle.stream.fsizer =
          NULL;  // You should provide a fsizer function if necessary
      h->handle.stream.closer =
          NULL;  // You should provide a closer function if necessary
      break;
  }
...
ZEND_INI_MH(php_beast_cache_size) {
  ...
  int length = (int)ZSTR_LEN(new_value);
  int retlen;
...
PHP_INI_BEGIN()
PHP_INI_ENTRY("beast.cache_size", "10485760", PHP_INI_ALL, php_beast_cache_size)
PHP_INI_ENTRY("beast.log_file", "./php-beast.log", PHP_INI_ALL,
              php_beast_log_file)
PHP_INI_ENTRY("beast.log_user", "root", PHP_INI_ALL, php_beast_log_user)
PHP_INI_ENTRY("beast.log_level", "notice", PHP_INI_ALL, php_beast_log_level)
PHP_INI_ENTRY("beast.enable", "1", PHP_INI_ALL, php_beast_enable)
...
#if BEAST_DEBUG_MODE
PHP_INI_ENTRY("beast.debug_path", "/tmp", PHP_INI_ALL, php_beast_debug_path)
PHP_INI_ENTRY("beast.debug_mode", "0", PHP_INI_ALL, php_beast_debug_mode)
#endif
PHP_INI_END()
...
static char *get_mac_address(char *networkcard) {
  ...
  PIP_ADAPTER_INFO pIpAdapterInfo =
      (PIP_ADAPTER_INFO)malloc(sizeof(*pIpAdapterInfo));
  if (!pIpAdapterInfo) {
    beast_write_log(beast_log_error,
                    "Failed to allocate memory for IP_ADAPTER_INFO");
    return NULL;
  }

  ret = GetAdaptersInfo(pIpAdapterInfo, &size);
  if (ERROR_BUFFER_OVERFLOW == ret) {
    // see ERROR_BUFFER_OVERFLOW
    // https://msdn.microsoft.com/en-us/library/aa365917(VS.85).aspx
    free(pIpAdapterInfo);
  ...
  if (ERROR_SUCCESS != ret) {
    beast_write_log(beast_log_error,
                    "Failed to get network adapter information");
    free(pIpAdapterInfo);
    return NULL;
  }
  ...
  do {
    if (strcmp(pCurrentAdapter->AdapterName, networkcard) == 0) {
      unsigned int i;  // Change 'int' to 'unsigned int'
      for (i = 0, pos = buf; i < pCurrentAdapter->AddressLength;
           i++, pos += 3) {
...
PHP_MINIT_FUNCTION(beast) {
  ...
  if (validate_networkcard() == -1) {
    php_error_docref(NULL, E_ERROR, "Not allow run at this computer");
    return FAILURE;
  }

  if ((encrypt_file_header_length + INT_SIZE * 2) > HEADER_MAX_SIZE) {
    php_error_docref(NULL, E_ERROR, "Header size overflow max size `%d'",
                     HEADER_MAX_SIZE);
    return FAILURE;
  }
  ...
  if (beast_cache_init(max_cache_size) == -1) {
    php_error_docref(NULL, E_ERROR, "Unable initialize cache for beast");
    return FAILURE;
  }

  if (beast_log_init(beast_log_file, log_level) == -1) {
    php_error_docref(NULL, E_ERROR, "Unable open log file for beast");
    return FAILURE;
  }
  ...
  if (getuid() == 0 && beast_log_user) { /* Change log file owner user */
    ...
    if (!pwd) {
      php_error_docref(NULL, E_ERROR, "Unable get user passwd information");
      return FAILURE;
    }

    if (beast_log_chown(pwd->pw_uid, pwd->pw_gid) != 0) {
      php_error_docref(NULL, E_ERROR, "Unable change log file owner");
      return FAILURE;
    }
  }
#endif
  ...
  REGISTER_LONG_CONSTANT("BEAST_ENCRYPT_TYPE_DES", BEAST_ENCRYPT_TYPE_DES,
                         CONST_CS | CONST_PERSISTENT);
  REGISTER_LONG_CONSTANT("BEAST_ENCRYPT_TYPE_AES", BEAST_ENCRYPT_TYPE_AES,
                         CONST_CS | CONST_PERSISTENT);
  REGISTER_LONG_CONSTANT("BEAST_ENCRYPT_TYPE_BASE64", BEAST_ENCRYPT_TYPE_BASE64,
                         CONST_CS | CONST_PERSISTENT);
 ...
 */
PHP_RSHUTDOWN_FUNCTION(beast) { return SUCCESS; }
/* }}} */
...
PHP_MINFO_FUNCTION(beast) {
  ...
  php_info_print_table_header(2, "beast V" BEAST_VERSION " support", "enabled");
...
PHP_FUNCTION(beast_file_expire) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "S", &input_file) == FAILURE) {
    RETURN_FALSE;
  }
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "s", &file, &file_len) ==
      FAILURE) {
    RETURN_FALSE;
  }
...
#endif
#ifdef PHP_WIN32
  fd = _open(file, O_RDONLY);  // Use _open on Windows
#else
  fd = open(file, O_RDONLY);  // Use open on other systems
#endif
  if (fd < 0) {
    goto error;
  }
...
#ifdef PHP_WIN32
  if (_read(fd, header, header_len) != header_len ||  // Use _read on Windows
      memcmp(header, encrypt_file_header_sign, encrypt_file_header_length)) {
    goto error;
  }
#else
  if (read(fd, header, header_len) !=
          header_len ||  // Use read on other systems
      memcmp(header, encrypt_file_header_sign, encrypt_file_header_length)) {
    goto error;
  }
#endif
...
#ifdef PHP_WIN32
  _close(fd);  // Use _close on Windows
#else
  close(fd);  // Use close on other systems
#endif
...
PHP_FUNCTION(beast_encode_file) {
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "SS|ll", &in, &out, &expire,
                            &encrypt_type) == FAILURE) {
    RETURN_FALSE;
  }
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS(), "ss|ll", &input, &input_len,
                            &output, &output_len, &expire,
                            &encrypt_type) == FAILURE) {
    RETURN_FALSE;
  }
  ...
  if (encrypt_type <= 0 || encrypt_type >= BEAST_ENCRYPT_TYPE_ERROR) {
    RETURN_FALSE;
  }

  ret = encrypt_file(input, output, (int)expire, (int)encrypt_type);
  if (ret == -1) {
    RETURN_FALSE;
  }
...
PHP_FUNCTION(beast_avail_cache) { RETURN_LONG(beast_mm_availspace()); }

PHP_FUNCTION(beast_support_filesize) { RETURN_LONG(beast_max_filesize); }

PHP_FUNCTION(beast_clean_cache) { beast_cache_flush(); }
