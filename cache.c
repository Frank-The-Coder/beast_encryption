...
void beast_cache_lock() { beast_spinlock(cache_lock, beast_pid); }

void beast_cache_unlock() { beast_spinunlock(cache_lock, beast_pid); }

static inline unsigned int beast_cache_hash(cache_key_t *key) {
  unsigned int retval;

  retval = (unsigned int)key->device * 3 + (unsigned int)key->inode * 7;
...
cache_item_t *beast_cache_find(cache_key_t *key) {
  ...
  while (item) {
    if (item->key.device == key->device && item->key.inode == key->inode) {
      break;
    }
    item = item->next;
  }
...
cache_item_t *beast_cache_create(cache_key_t *key) {
  ...
  if (!item) {
    beast_write_log(beast_log_notice, "Not enough memory for alloc cache");
    return NULL;
  }
